...
func main() {
	...
	config, _ := track.Track(
		track.WithConfigTag("service", "golang-gin-stack"),
		...
		// track.WithConfigTag("accessToken", "xtzrzvturlisziuvpdffwdsjavkwzpyplwrz"),
		// track.WithConfigTag("target", "kbuin.beta.env.middleware.io:443"),
		track.WithConfigTag(track.Debug, true),
		track.WithConfigTag(track.PauseMetrics, true),
	)

	r.Use(g.CombinedMiddlewareWithOtel(config)...)
